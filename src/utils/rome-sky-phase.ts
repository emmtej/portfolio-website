...
const romeTimeFmt = new Intl.DateTimeFormat("en-GB", {
  timeZone: ROME_TZ,
  hour: "numeric",
  minute: "numeric",
  hourCycle: "h23",
...
const romeTimePartsFmt = new Intl.DateTimeFormat("en-GB", {
  timeZone: ROME_TZ,
  hour: "numeric",
  minute: "numeric",
  hourCycle: "h23",
...
/**
 * True when `timeZone` has the same UTC offset as Europe/Rome at `instant`
 * (local clock matches Rome, including across DST changes).
 */
export function isTimeZoneOffsetEqualToRome(instant: Date, timeZone: string): boolean {
  if (!timeZone) return false;
  return getTimezoneOffset(timeZone, instant) === getTimezoneOffset(ROME_TZ, instant);
}
...
function getRomeMinutesSinceMidnight(instant: Date): number {
  const parts = romeTimePartsFmt.formatToParts(instant);
...
export function formatRomeTime(instant: Date): string {
  return romeTimeFmt.format(instant);
}
...
function nextRomeCalendarYmd(ymd: string): string {
  ...
  for (let step = 0; step < 30; step++) {
    t += 3600000;
    const next = romeDayFmt.format(new Date(t));
    if (next !== ymd) {
      return next;
    }
  }
