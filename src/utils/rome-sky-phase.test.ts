...
import {
  ...
  isTimeZoneOffsetEqualToRome,
} from "./rome-sky-phase";

describe("isTimeZoneOffsetEqualToRome", () => {
  it("is true for Europe/Rome", () => {
    const instant = new Date("2026-06-15T12:00:00.000Z");
    expect(isTimeZoneOffsetEqualToRome(instant, "Europe/Rome")).toBe(true);
  });

  it("is true for another zone with the same offset as Rome (e.g. Berlin in summer)", () => {
    const instant = new Date("2026-06-15T12:00:00.000Z");
    expect(isTimeZoneOffsetEqualToRome(instant, "Europe/Berlin")).toBe(true);
  });

  it("is false when offsets differ", () => {
    const instant = new Date("2026-06-15T12:00:00.000Z");
    expect(isTimeZoneOffsetEqualToRome(instant, "America/New_York")).toBe(false);
  });

  it("is false for empty time zone", () => {
    expect(isTimeZoneOffsetEqualToRome(new Date(), "")).toBe(false);
  });
});
