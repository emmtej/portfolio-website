...
export type RomeAvailabilityState = {
  ...
  localRange: string;
  /** False when the visitor's timezone offset matches Rome (redundant to show "Local: …"). */
  showLocalRange: boolean;
...
export function useRomeAvailability(intlLocale: string): RomeAvailabilityState {
  ...
  return useMemo(() => {
    ...
    const showLocalRange = !isTimeZoneOffsetEqualToRome(instant, localTimeZone);
