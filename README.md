...
- Static routes at `/` (about), `/development`, `/audio`, and `/contact`; same-language tab navigation switches panels without reloading
...
├── utils/            # Class names, translations, and Rome sky calculations
└── index.css         # Global styles / Tailwind layers
public/
├── favicon.svg       # SVG favicon
└── site.webmanifest  # Web app metadata
scripts/             # Security header generation and build verification
...
| `npm run test:e2e` | Run browser tests, build, then test the production CSP |

---

## Deployment

Netlify build and redirect configuration lives in [`netlify.toml`](netlify.toml).

- Build command: `npm run build`
- Publish directory: `dist/`
- Security headers: CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy

GitHub Actions runs Gitleaks secret scanning on pushes and pull requests to `main`.
Run lint, typecheck, and tests locally; they are not part of CI.
